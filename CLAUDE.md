...
| Motor | `src/wallrotate/engine.py` | Revisa perfiles, aplica el siguiente fondo si venció el intervalo. Se ejecuta vía systemd timer. Si `config.pause_on_fullscreen`, salta el chequeo (no afecta `force=True`) consultando `fullscreen.py` |
| Deteccion pantalla completa | `src/wallrotate/fullscreen.py` | Carga un script de KWin (`resources/check_fullscreen.js`) por D-Bus que lee `workspace.activeWindow.fullScreen` y llama de vuelta via `callDBus` a un servicio D-Bus temporal (`QDBusConnection` + `ExportAllSlots`, interfaz vacia en el `callDBus` del lado JS). Funciona en X11 y Wayland porque no inspecciona ventanas desde afuera, le pregunta al compositor. Falla "cerrado" (False) ante cualquier error |
